...
- Device Token Bindings 列表只展示 mask 后的 token，管理员可以点击 `Copy` 复制完整 token。
...
```sh
java -jar target/agent-dashboard-0.1.0-SNAPSHOT.jar \
...
- `Device Token Bindings` 列表会显示 mask 后的 token，例如 `abc123****wxyz`。
...
- collector 默认使用 `~/.agent-dashboard/collector-sqlite` 保存本机 checkpoint，不需要显式传 `--collector-db`。
...
- `--collector-db`: 覆盖 collector checkpoint 目录；默认 `~/.agent-dashboard/collector-sqlite`。
...
- collector checkpoint 默认就是独立目录 `~/.agent-dashboard/collector-sqlite`。
...
```json
{"status":"ok","events":12,"batches":1,"accepted":12,"duplicate":0,"rejected":0}
```

如果重复运行，可能看到：

```json
{"status":"ok","events":12,"batches":1,"accepted":0,"duplicate":12,"rejected":0}
```
...
安装脚本默认会从脚本同目录寻找 `agent-dashboard-0.1.0-SNAPSHOT.jar`。如果 teammate 的 Java 不在标准 PATH 中，脚本会优先使用 `JAVA_HOME/bin/java`，也可以显式传 `AGENT_DASHBOARD_JAVA`。
...
### 7.6 服务端和 collector 使用了不同 DB

服务端 `--db` 是中央库。

collector checkpoint 默认是成员本机 `~/.agent-dashboard/collector-sqlite`，也可以用 `--collector-db` 覆盖。

它们不需要相同，但 Team 页面只读取服务端 `--db` 里的 team shard：
...
输出里的 `device_token` 可直接发给成员。后续如果通过 admin 页面管理新建 token，也可以在 Device Token Bindings 列表点击 `Copy` 复制。成员机器仍然需要运行 `--collect-team` 上报。
